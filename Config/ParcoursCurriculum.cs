...
/// (frapper) et chapitre 3 (bouger), mécaniques sourcées explicitement — voir SourceNote sur chaque
/// leçon concernée. Chapitre 3 : le plan prévoyait 4 leçons (dash, dash jump, backdash, dodge
/// directionnel comme outil de déplacement) mais l'utilisateur a clarifié que la 4ᵉ n'est pas une
/// mécanique séparée — l'esquive directionnelle aérienne est déjà couverte par la leçon 1.1, son
/// usage comme "option de recover en plus" est un simple à-côté, pas de leçon dédiée pour éviter la
/// redondance. Les chapitres 4 et 5 du plan (premier vrai combo, techniques avancées) ne sont
/// volontairement pas ici : à sourcer progressivement de la même façon plutôt qu'en un seul lot
/// (voir CLAUDE.md, "Version 13"/"Version 14"/"Version 15"/"Version 16").
///
